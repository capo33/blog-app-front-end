...
const Profile = () => {
  ...
  const data = {
    ...
    about: auth?.user?.user?.about,
    interests: auth?.user?.user?.interests,
    createdAt: auth?.user?.user?.createdAt,
    role: auth?.user?.user?.role,
    blogs: auth?.user?.user?.blogs,
  ...
  const handleDeleteProfile = async () => {
    dispatch(logout());
    dispatch(deleteUser({ token: data?.token, toast, navigate }));
  };
